...
class Pagination extends Config {
	...
	public static function create($rows) {
		...
		{
			if($prev != 0)
				$pagination.= "<li class='previous_page'><a href='".self::$_PAGE_URL.self::$_url[0]."/page/1'>« First</a></li>";  
			else 
			...
			{   
				for ($counter = 1; $counter <= $lastpage; $counter++)
				{
					if ($counter == self::$_url[2])
						...
						$pagination.= "<li><a href='".self::$_PAGE_URL.self::$_url[0]."/page/$counter'>$counter</a></li>";                   
				}
			}
			elseif($lastpage > 5 + ($adjacents * 2))
			{
				if(self::$_url[2] < 1 + ($adjacents * 2))       
				{
					...
					$pagination.= "<li><a href='".self::$_PAGE_URL.self::$_url[0]."/page/$lpm1'>$lpm1</a></li>";
					$pagination.= "<li><a href='".self::$_PAGE_URL.self::$_url[0]."/page/$lastpage'>$lastpage</a></li>";       
				}
				elseif($lastpage - ($adjacents * 2) > self::$_url[2] && self::$_url[2] > ($adjacents * 2))
				{
					$pagination.= "<li><a href='".self::$_PAGE_URL.self::$_url[0]."/page/1'>1</a></li>";
					$pagination.= "<li><a href='".self::$_PAGE_URL.self::$_url[0]."/page/2'>2</a></li>";
					$pagination.= "<li class='dots'><a href='#'>...</a></li>";
			...
			if($lastpage == (isset(self::$_url[2]) ? self::$_url[2] : 1))
				...
				$pagination.= "<li class='next_page'><a href='".self::$_PAGE_URL.self::$_url[0]."/page/$lastpage'>Last »</a></li>";  
		}
		$pagination .= "</ul>";
		return $pagination;
	}
