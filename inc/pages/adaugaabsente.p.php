...
<div class="login-container" >
	...
	<div class="position-relative">
		<div id="login-box" class="login-box visible widget-box  round1" >
			<div class="widget-header widget-header-flat" style="border-top: 1;border-bottom: 1px solid #eee;">
				<h4 class="smaller"><span>Adauga o absenta lui <?php echo $data->name; ?></span>
					...
					<form name="login_form" id="login_form" action="" method="post">
						<label class="block clearfix">
							<span class="block input-icon input-icon-right">
								<input type="text" class="form-control" placeholder="Absenta" name="nota_nota">
						...
						<center>
							<div class="clearfix">
								<input type="submit" name="login_submit" class="width-35 btn btn-sm btn-primary no-border" value="Adaugare Absenta">
							</div>
						</center>
	...
	if(isset($_POST['login_submit'])) {
		$nume = $data->name;
		$data = $_POST['nota_nota'];
		$selectOption = $_POST['taskOption'];
		if($selectOption!=0)
		{
			$q = DB::prepare('INSERT INTO `absente` (`Nume`,`Absenta`,`Materie`) VALUES (?,?,?)');
			$q->execute(array($nume,$data,$selectOption));
			echo '<h3><center><font color=green>Absenta adaugata cu succes!</font><br><strong>'.$data.'</strong><br>Inapoi la profilul lui '.$nume.'?<br><a href="'.Config::$_PAGE_URL.'/profile/'.$user.'"><button>Da</button></center></h3>';
