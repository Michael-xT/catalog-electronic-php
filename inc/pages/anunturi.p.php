		...
		<div class="box-body">
			...
			if(isset($_POST['create_topic'])) {
				...
				if(!strlen($_POST['editor1']) || !strlen($_POST['version'])) { echo "<center><font color='red'>Completeaza <strong>toate</strong> campurile cu mai multa atentie!</font></center><br>"; } else {
					...
					$q = DB::prepare('INSERT INTO `anunturi` (`UpdateVersion`,`Content`,`Author`) VALUES (?,?,?)');
					$q->execute(array($_POST['version'],$_POST['editor1'],User::get()));
				}
			}
			if(isset($_POST['remove_topic'])) 
			{
				...
				$q = DB::prepare('DELETE FROM `anunturi` WHERE `ID`=?');
			...
			<table class="table table-bordered table-striped">
				...
				<?php $q = DB::prepare('SELECT * FROM `anunturi` ORDER BY `ID` DESC');
				$q->execute();
			...
			<?php } else if(isset(Config::$_url[1]) && isset(Config::$_url[1]) !== "new") { 
				echo '<div class="well">';
				$q = DB::prepare('SELECT * FROM `anunturi` WHERE `ID`=?');
				$q->execute(array(Config::$_url[1]));
				while($row = $q->fetch(PDO::FETCH_OBJ)) 
				{
					?> <title><?php echo $row->UpdateVersion; ?></title> <?php
					echo '
					<div class="col-md-6"><p><strong>de <i>' . User::getData($row->Author,"name") . '</i></p></strong></div>
					<div class="col-md-6"><p class="pull-right"><i class="fa fa-table"></i> Postat pe data de '.$row->Time.'</p></div><br><br>
